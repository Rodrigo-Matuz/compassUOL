...
Fazer Requisição API
    ...
    [Arguments]    ${method}    ${endpoint}    ${body}=${EMPTY}    ${token}=${EMPTY}    ${expected_status}=any
    ...
    ...    POST On Session    api-session    url=${endpoint}    json=${body}    headers=${headers}    expected_status=${expected_status}
    ...    ELSE IF    '${method}' == 'GET'
    ...    GET On Session    api-session    url=${endpoint}    headers=${headers}    expected_status=${expected_status}
    ...    ELSE IF    '${method}' == 'PUT'
    ...    PUT On Session    api-session    url=${endpoint}    json=${body}    headers=${headers}    expected_status=${expected_status}
    ...    ELSE IF    '${method}' == 'DELETE'
    ...    DELETE On Session    api-session    url=${endpoint}    headers=${headers}    expected_status=${expected_status}
